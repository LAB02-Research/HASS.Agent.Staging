...
namespace HASS.Agent.Shared.Managers
{
    ...
    {
        /// <summary>
        /// Executes the file within a command prompt, parses and returns the output
        /// <para>Uses default timeout of 5 minutes</para>
        /// </summary>
        /// <param name="file"></param>
        /// <param name="args"></param>
        /// <returns></returns>
        public static async Task<ConsoleResult> ExecuteCommandAsync(string file, string args)
        {
        ...
        /// <returns></returns>
        public static async Task<ConsoleResult> ExecuteCommandAsync(string file, string args, TimeSpan timout)
        {
        ...
        /// <returns></returns>
        public static bool ExecuteHeadless(string command)
        {
            try
            {
        ...
        /// <summary>
        /// Executes the file elevated, waits max 5 minutes for it to exit and returns whether the exitcode equals 0
        /// </summary>
        /// <param name="file"></param>
        /// <param name="arguments"></param>
        /// <returns></returns>
        public static bool ExecuteElevated(string file, string arguments = "") => ExecuteElevated(file, arguments, TimeSpan.FromMinutes(5));

        /// <summary>
        /// Executes the file elevated, waits for it to exit and returns whether the exitcode equals 0
        /// </summary>
        /// <param name="file"></param>
        /// <param name="arguments"></param>
        /// <param name="timeOut"></param>
        /// <returns></returns>
        public static bool ExecuteElevated(string file, string arguments, TimeSpan timeOut)
        {
            try
            {
                // prepare startinfo
                ...
                if (process.HasExited == false)
                {
                    ...
                    {
                        Log.Error("[CLI] File hasn't finished within timeout, but was still responding: {file}", file);
                        process.CloseMainWindow();
                    }
                    else
                    {
                        Log.Error("[CLI] File hasn't finished within timeout, stopped responding: {file}", file);
                        process.Kill();
                    }
                }
            ...
            {
                Log.Fatal(ex, "[CLI] Fatal error when executing file elevated: {file}", file);
                return false;
            }
        }
        ...
        public static void ExecuteProcessUnElevated(string file, string args = "")
        {
            ...
            shellDispatch.ShellExecute(file, args, string.Empty, string.Empty, SW_SHOWNORMAL);
        }
    ...
    internal class NativeMethod
    {
        ...
        internal const uint SE_GROUP_INTEGRITY = 0x00000020;
        ...
        /// </returns>
        [DllImport("advapi32.dll", CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern IntPtr GetSidSubAuthority(
            IntPtr pSid,
            uint nSubAuthority);
    ...
    internal class KnownFolder
    {
        ...
        /// </param>
        /// <returns></returns>
        internal static string SHGetKnownFolderPath(Guid rfid)
        {
